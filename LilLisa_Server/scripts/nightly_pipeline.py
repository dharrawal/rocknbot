...
built in earlier steps:
    1. nightly_techsupport_sync.sync() -- detect new/updated threads in the
       ...
       scripts/env/techsupport_sync.env; the same channel is used for IDA,
...
channel (ADMIN_CHANNEL_ID in scripts/env/techsupport_sync.env -- reuses the
...
scripts/env/github_push.env) -- but only when this run actually added,
...
def process_thread(thread_ts: str, token: str, channel_id: str, already_added: bool) -> str:
    ...
    if related_entry_title:
        try:
            ...
            logger.warning(
                "Thread %s tagged related_entry_title=%r but enrich failed (%s) -- "
                "falling back to normal add",
                thread_ts, related_entry_title, exc,
            )
